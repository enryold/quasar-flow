...
public class OkHttpProcessor<T> extends AbstractIOProcessor<QHTTPRequest<T>, QHTTPResponse<T>> {

    private OkHttpClient okHttpClient = new FiberOkHttpClient();

    public OkHttpProcessor(IEmitter<QHTTPRequest<T>> eEmitter, String name, QRoutingKey routingKey) {
        super(eEmitter, name, routingKey);
        init();
    }

    public OkHttpProcessor(IEmitter<QHTTPRequest<T>> eEmitter, QRoutingKey routingKey) {
        super(eEmitter, routingKey);
        init();
    }

    public OkHttpProcessor(IEmitter<QHTTPRequest<T>> eEmitter) {
        super(eEmitter);
        init();
    }
